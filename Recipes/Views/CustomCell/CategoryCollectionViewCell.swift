...
extension CategoryCollectionViewCell: CategoryViewInterface {
    ...
    func setLayout() {
        image.anchor(top: self.topAnchor, leading: self.leadingAnchor, bottom: nil, trailing: nil, padding: .init(top: 0, left: 5, bottom: 0, right: 0), size: .init(width: 50, height: 50))
        label.anchor(top: self.topAnchor, leading: image.trailingAnchor, bottom: nil, trailing: nil, padding: .init(top: 0, left: 0, bottom: 0, right: 0), size: .init(width: 80, height: 50))
    }
    func configure(with model: CocktailPresentation) {
